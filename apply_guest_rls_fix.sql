-- Apply Guest Checkout RLS Policy Fix
...
-- Step 2: Drop order_items policies that also need fixing
DROP POLICY IF EXISTS "Users can insert their own order items" ON public.order_items;
DROP POLICY IF EXISTS "Users can view their own order items" ON public.order_items;

-- Step 3: Create new policies that properly handle guest orders
...
-- Step 4: Create order_items policies that work with both authenticated and guest orders
...
-- Step 6: Test query to verify guest order constraint
-- This should show the constraint that ensures either user_id or guest info is provided
